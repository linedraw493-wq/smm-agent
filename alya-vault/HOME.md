...
- [[kontent-plan-instagram]] — **контент-план Instagram**: разбор чужих IT-компаний, 5+1 рубрика, ритм недели, сетка на месяц. Принят владельцем 2026-08-28, `active`.
- [[nedelya-2026-08-31]] — **план недели 31.08–06.09**: три ролика, готовые сценарии, одна посадка на съёмку. Заказ владельца 2026-08-30.
...
[[decisions/2026-08-25-muzyka-ploshchadki]] — музыка от площадки (**поправлено**) ·
[[decisions/2026-08-25-muzyka-svoya]] — **музыку вшиваем, и делаем сами** ·
[[decisions/2026-08-25-masterskaya-rasshirena]] — **мастерская: кадр, документы, интерфейсы**.

## Что лежит не в vault

- `../tools/` — код, который исполняет ремесло. С 2026-08-25 плюс пять
  модулей: `fonts.py` (реестр шрифтов), `frame.py` (сплит, подложка,
  указатель), `doc.py` (документы), `maket.py` (макеты экранов) и
  **`music_make.py` — своя музыка синтезом** (пэд без ударных, три
  настроения; прав нет ни у кого, как и у SFX).
...
- `../projects/` — выпуски: сценарий, сырьё, работа, готовое, `run.md`.
  ...
  `work/sky.py`). **`2026-08-25-m4ksi-den-osnovatelya` — первый ролик,
  собранный целиком:** `out/reel.mp4`, 27.13 с, живой голос владельца,
  своя музыка, вшитые субтитры, пять машинных гейтов и шестой глазами.
  Внутри разбор сырья `razbor.md`, сценарий `script.md` (история решения —
  записанный текст с ним не совпал) и `run.md` с числами звука было/стало.
  Наружу не вышел ни один.
- `../assets/` — шрифты, музыка, SFX, логотипы (вне git). В `music/` —
  своя подложка `calm-day-32s.wav`, реестр `licenses.md` в git.
