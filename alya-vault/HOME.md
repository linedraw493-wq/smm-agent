...
- `../projects/` — выпуски: сценарий, сырьё, работа, готовое, `run.md`.
  ...
  `work/sky.py`). `2026-08-25-m4ksi-den-osnovatelya` — сырьё без звука
  (парковка → машина → дорога → работа в парке → потягивание в беседке),
  контур M4ksi, задача бренд: разбор `razbor.md` + **первый настоящий
  сценарий с раскадровкой** `script.md` (16 планов, 28.7 с, крючок задан
  словом владельца, реплики начисто). Сборки нет — три бита ждут
  досъёмки. Наружу не вышел ни один.
