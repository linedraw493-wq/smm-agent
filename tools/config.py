...
LUFS_TARGET = -14
TRUE_PEAK = -1
# Диапазон громкости. Было 11 — это вещательная норма, для ленты она широкая:
# тихие места проваливаются в шум телефона на улице.
#
# Замер 2026-08-23 по пяти рилсам блогеров ниши: LRA 2.0 · 2.0 · 2.4 · 2.5 ·
# 3.1. Плюс более ранний разбор референса дал 2.1. То есть короткое видео
# звучит ПЛОТНО, почти без динамики, и это не небрежность, а формат.
#
# Ставим 5, а не 2: loudnorm сжимать не умеет, он умеет только не мешать.
# Динамику даёт компрессор в audio_polish ДО него. Требовать 2 от loudnorm —
# получить артефакты, о которых предупреждает craft/audio.
LRA = 5
