...
def build(profile, space, rnnoise, lufs, bass=0.0, headroom=0.0, comp=2.6,
    ...
    # Мутность 300-650 Гц. Штатный де-мут — одна точка на 450 Гц, и на записи,
    # где мутная полоса перевешивает тело голоса, её не хватает: вырез узкий,
    # а перевес широкий. Ключ --mud добавляет ещё две точки, 360 и 580 Гц, и
    # вместе с 450 они кроют полосу целиком.
    #
    # ⚠️ Полоса 300-650 — это НЕ только муть, там же нижняя опора голоса.
    # Убрать её всю значит получить телефонный, пустой звук. Замер 2026-08-25:
    # цель — привести полосу к уровню тела голоса (500-2k), а не ниже.
    # Мерить audio_report.py, строка «муть 200-500» относительно тела.
    # Три точки, а не две: замер 2026-08-25 показал, что двумя вырезами
    # (360 и 580, Q1.5) полоса 300-650 просела всего на 1.8 dB из нужных 4.5 —
    # между точками оставался горб. Третья точка на 460 закрывает середину,
    # верхняя опущена до 620, чтобы меньше задевать тело голоса выше 650.
    if mud:
        for f0 in (330, 460, 620):
            chain.append("equalizer=f=%d:t=q:w=1.8:g=%.2f" % (f0, -abs(mud)))
...
def main():
    ...
    ap.add_argument("--mud", type=float, default=0.0,
                    help="дополнительный вырез мутной полосы 300-650 Гц, dB "
                         "(две точки, 360 и 580, поверх штатного 450). Ставить "
                         "по замеру: цель — сравнять полосу с телом голоса, "
                         "не увести ниже, иначе голос станет пустым")
